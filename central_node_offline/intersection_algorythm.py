...
def intersection(data_matrix):
    ...
    azimuth1 = data_matrix[0][0]
    elevation1 = data_matrix[0][1]
    azimuth2 = data_matrix[1][0]
    elevation2 = data_matrix[1][1]
...
with open('200.txt', 'r') as file1:
    data1 = [list(map(float, line.split())) for line in file1]

# Read data from file2.txt
with open('150.txt', 'r') as file2:
    data2 = [list(map(float, line.split())) for line in file2]

# Combine data from both files into a single matrix
matrix = [[row1, row2] for row1, row2 in zip(data1, data2)]
