...
theta = np.radians([np.rad2deg(np.arctan(10/15)), np.rad2deg(np.arctan(10/150)), np.rad2deg(np.arctan(10/360))])
radius = [15, 150, 360]
#error = [2.07, 3.1, 8.69]
error = [3.85, 20.14, 20.71]
...
theta_grid, radius_grid = np.meshgrid(np.linspace(0, np.pi, 1000), np.linspace(0, 500, 1000))

# Interpolate errors onto the grid
error_grid = griddata((all_theta, all_radius), all_error, (theta_grid, radius_grid), method='linear', fill_value=35) #20 albo 35
...
contour = ax.contourf(theta_grid, radius_grid, error_grid, cmap='jet', levels=1000, vmin=0, vmax=30)    #15 albo 30

# Set theta limits to plot only half of the polar plot
ax.set_thetamin(0)
ax.set_thetamax(180)

# Set radial axis limits for better visibility
ax.set_rmax(500)  # Adjust as needed

# Set colorbar
cbar = plt.colorbar(contour, ax=ax, label='Błąd bezwzględny [$^{\circ}$]')

# You can customize the polar plot if needed
plt.title("Charakterystyka kąta elewacji węzła akustycznego wyznaczona na podstawie interpolacji błędu bezwzględnego")
plt.grid(True)
